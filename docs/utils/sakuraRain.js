...
let Sakura = function Sakura(selector, options) {
    ...
    this.createPetal = function () {
        ...
        let fallTime = (document.documentElement.clientHeight * 0.007 + Math.round(Math.random() * 5)) * _this.settings.fallSpeed // Create animations
        ...
        let animations = animationsArr.join(', ') // Create petal and give it a random size.

        let petal = document.createElement('div')
        petal.classList.add(_this.settings.className)
        let height = randomInt(_this.settings.minSize, _this.settings.maxSize)
        let width = height - Math.floor(randomInt(0, _this.settings.minSize) / 3) // Get a random color.

        let color = randomArrayElem(_this.settings.colors)
